 ...
 *   2026-09-16 在這台機器實測出來的雷:**Node 24 的 `fs.rm*` 只要「目前工作目錄的路徑含非 ASCII 字元」
 ...
 *
 *   實測對照(同一台機、同一個 node v24.13.0):
 *     · cwd = …\scratchpad\        (純 ASCII) → fs.rmSync 正常
 *     · cwd = …\scratchpad\0916早test\        → fs.rmSync **當掉**(連刪單一檔案都當)
 *     · cwd = …\Downloads\zz_probe_ascii\     → fs.rmSync 正常
 *     · 任何 cwd:`unlinkSync` + `rmdirSync` **都正常**
 *
